...
valor = []
for x in range(1, 4):
    valor.append(float(input(f'entre com um valor: ')))
...
vlr_2 = []
for x in range(1, 4):
    vlr_2.append(float(input(f'entre com um valor: ')))
...
def soma_3(a: float, b: float, *args) -> float:  # args = tupla ()
    # transforma em lista e junta a lista com o args | [list() # converte]
    vl_3 = [a, b] + list(args)
    return sum(vl_3)
